...
```
  ...
  ─────────────────────► │  (ルーティング / 認証検証)    │
   ...
   │  JWT検証DB  │  │                MySQL (RDS)                               │
   │  (Redis)    │  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐ │
   │（ブラックリ  │  │  │  tickets DB  │  │  masters DB  │  │  analytics DB   │ │
   │  スト管理） │  │  │  tickets     │  │  customers   │  │  (将来: 読み取   │ │
...
権限チェックはサービス内で `X-User-Role` ヘッダーを参照して行う。内部サービスへの直接アクセスはVPCネットワーク制御で防ぐ。
